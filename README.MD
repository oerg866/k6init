...
(C) 2021 - 2026, Eric Voirin (oerg866)
...
**K6INIT** is a driver for MS-DOS that lets you configure special features of AMD K5/K6/K6-2/2+/3/3+ processors, replacing tools like `k6dos.sys` and `setk6.exe`, and is similar to FASTVID on Pentium systems.
...
- *Write Combining* for **Linear Frame Buffer(s) exposed by VESA BIOS or prefetchable memory regions from PCI/AGP graphics cards directly**
...
**K6INIT** runs on AMD K5 chips or later, but K5, K6 and K6-2 (pre-CXT) only have basic functionality. More in-depth configuration options are available with K6-2 (CXT) and later.
...
For more information, check [the FBTWEAK documentation.](FBTWEAK.MD)
...
|-------------------------|--------|-------|----------|
| AMD K5 w/ Write Allocate| 5      | 0..3  | 4..F     |
| AMD K6                  | 5      | 6 / 7 | *Any*    |
| AMD K6-2                | 5      | 8     | 0..7     |
...
* Supported AMD K5/6 family CPU (see above)
* Microsoft MS-DOS 5.0 (or compatible) or higher
    * [X] Microsoft Windows 95/98 support
    * [X] FreeDOS support
    * [X] IBM PC DOS 7.x/2000 support
    * Other systems such as DR-DOS untested, but should work, your mileage may wary when loading it from CONFIG.SYS though.
...
---
### `/forcenonpf`
**Description:** Use PCI/AGP FBs that are not marked prefetchable

Usually framebuffers in PCI/AGP devices are marked 'prefetchable'. By default, BARs that do not, are ignored.

Use this parameter to force their usage.
...
### Required toolchain and library

Building requires Microsoft C/C++ 7.00 or 8.00 (from MSVC 1.52). The latter allows building on modern windows systems, as the toolchain uses very early 32-Bit executables.

The `MSC_Libs` folder must be copied on top of that installation. It contains the late Geoff Chappell's libraries for writing device drivers in C, from his book ["DOS Internals"](https://archive.org/details/dosinternals) - plus a fix for IBM PC DOS 7.xx.
